function import_keras_network(keras_model, keras_model_meta, buffer){
    ...
    for(let layer of keras_model.config.layers){
        ...
        function W(name){ return getWeight(layer.name + '/' + name) }
        ...
        if(layer.class_name == 'Conv2D'){
            ...
            if(ndops.norm1(W('bias:0')) < 1e-5){
                network.push({
                    name: layer.name,
                    type: 'Convolve2D',
                    subsample: layer.config.strides,
                    border_mode: layer.config.padding,
                    kernel: W('kernel:0'),
                    deps: {
                        image: inbound[0]
                    },
            ...
            console.assert(ndops.norm1(W('bias:0')) < 1e-5) // bias must be zero
            // console.assert(layer.config.dim_ordering == 'tf')
            debugger
            // network.push({
            //     name: layer.name,
            //     type: 'Deconvolve2D',
            //     kernel: W('kernel:0'),
            //     subsample: layer.config.subsample || alert('error'),
            //     padding: [2, 2],
            //     deps: {
            //         image: inbound[0]
            //     },
            ...
            // console.assert(ndops.norm1(W('bias:0')) < 1e-5)
            debugger
            network.push({
                name: layer.name,
        ...
        }else if(layer.class_name == 'Concatenate'){
